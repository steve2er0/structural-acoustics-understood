...
const extraCalculatorDefinitions = {
  ...
  'duration-scaling': {
    ...
    compute(v){const G=Math.max(0,n(v.psd)),T1=positive(v.duration1,'Reference duration'),T2=positive(v.duration2,'Target duration'),b=positive(v.b,'S–N exponent'),r=Math.max(0,n(v.rms)),psdRatio=(T1/T2)**(2/b),rmsRatio=(T1/T2)**(1/b);return{summary:[stat('Target PSD level',G*psdRatio,'unit²/Hz'),stat('PSD scale factor',psdRatio),stat('Target RMS',r*rmsRatio,'response units'),stat('RMS scale factor',rmsRatio),stat('Duration ratio',T2/T1)],interpretation:`For b=${b}, changing duration from ${T1} s to ${T2} s gives an equal-damage PSD factor of ${psdRatio.toFixed(4)}.`,warnings:['Broadband fatigue depends on spectral moments, mean stress, cycle counting, non-Gaussianity, material scatter, and stress transfer. Use this only as a stated screening approximation.']};}
  },
  ...
  'panel-cavity': {
    category:'Structural Acoustics',basis:'In-vacuo plate and rigid-cavity mode proximity',confidence:'Screening calculation',
    inputs:[{key:'E',label:'Young’s modulus',unit:'GPa',type:'number',default:68.9,min:.001},{key:'rho',label:'Panel density',unit:'kg/m³',type:'number',default:2700,min:.001},{key:'nu',label:'Poisson ratio',type:'number',default:.33,min:-.9,max:.49},{key:'thickness',label:'Panel thickness',unit:'mm',type:'number',default:3,min:.001},{key:'a',label:'Panel length',unit:'m',type:'number',default:1,min:.001},{key:'b',label:'Panel width',unit:'m',type:'number',default:.6,min:.001},{key:'depth',label:'Cavity depth',unit:'m',type:'number',default:.5,min:.001},{key:'max_index',label:'Maximum modal index',type:'number',default:5,min:2,max:8,step:1},{key:'tolerance',label:'Frequency tolerance',unit:'%',type:'number',default:10,min:0}],
    theory:'<p>Simply supported plate modes are compared with rigid-wall rectangular cavity modes. Small frequency detuning is necessary but not sufficient for strong coupling.</p>',assumptions:['Thin isotropic simply supported plate.','Rigid rectangular cavity; in-vacuo plate frequencies and no added fluid mass.'],example:'Changing cavity depth sweeps axial acoustic modes through fixed structural modes.',
    compute(v){const E=positive(v.E,'Modulus')*1e9,rho=positive(v.rho,'Density'),nu=n(v.nu),h=positive(v.thickness,'Thickness')/1000,a=positive(v.a,'Length'),b=positive(v.b,'Width'),d=positive(v.depth,'Depth'),N=clamp(Math.round(n(v.max_index,5)),2,8),tol=Math.max(0,n(v.tolerance)),D=E*h**3/(12*(1-nu*nu)),mA=rho*h,sm=[],am=[];for(let m=1;m<=N;m++)for(let q=1;q<=N;q++){const f=Math.PI/(TWO_PI)*Math.sqrt(D/mA)*((m/a)**2+(q/b)**2);sm.push({id:`P(${m},${q})`,f,m,q});}for(let nx=0;nx<=N;nx++)for(let ny=0;ny<=N;ny++)for(let nz=0;nz<=N;nz++){if(nx+ny+nz===0)continue;const f=AIR_C/2*Math.sqrt((nx/a)**2+(ny/b)**2+(nz/d)**2);am.push({id:`A(${nx},${ny},${nz})`,f,nx,ny,nz});}const pairs=[];for(const s of sm)for(const aa of am){const det=100*Math.abs(s.f-aa.f)/((s.f+aa.f)/2);if(det<=tol)pairs.push([s.id,s.f,aa.id,aa.f,det]);}pairs.sort((x,y)=>x[4]-y[4]);return{summary:[stat('Plate modes',sm.length),stat('Cavity modes',am.length),stat('Pairs within tolerance',pairs.length),stat('Closest detuning',pairs.length?pairs[0][4]:NaN,'%')],interpretation:pairs.length?`${pairs[0][0]} and ${pairs[0][2]} are the closest pair at ${pairs[0][4].toFixed(2)}% detuning. Spatial overlap still controls actual coupling.`:'No pairs fall inside the selected tolerance.',warnings:['Frequency proximity alone does not establish coupling strength. Compute interface modal overlap and include fluid loading in a coupled model.'],tables:[{title:'Near-frequency mode pairs',columns:['Plate mode','Plate f (Hz)','Cavity mode','Cavity f (Hz)','Detuning (%)'],rows:pairs.slice(0,100)}]};}
  },
