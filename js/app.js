...
const chapterHardwareLinks = [
  { title: 'Payload fairing', description: 'Cavity and panel coupling', href: '#/cheat-sheet?section=payload-fairing-cavities' },
  { title: 'Wet tank barrels', description: 'Fluid-loaded shell dynamics', href: '#/cheat-sheet?section=wet-tank-dynamics-deep-dive' },
  { title: 'Intertank transitions', description: 'Impedance and wave matching', href: '#/cheat-sheet?section=wave-matching-deep-dive' },
  { title: 'Launch source region', description: 'Forcing entering the stack', href: '#/cheat-sheet?section=launch-acoustic-sources-deep-dive' }
];
...
function renderCheat(route){
  const proof=isChapterDesignProof(route);
  ...
  const content=sections.map(section=>{
    const selectedProof=proof&&section.id===DESIGN_PROOF_CHAPTER_ID;
    const header=renderSectionHeader({
      number:section.number,
      eyebrow:section.eyebrow,
      title:section.title,
      summary:section.summary,
      action:section.deepDiveId?{label:'Read the complete launch-vehicle deep dive',href:`#/case-note/${encodeURIComponent(section.deepDiveId)}`}:null
    });
    const hardware=selectedProof?renderLinkCollection({label:'Hardware topics',items:chapterHardwareLinks,variant:'hardware'}):'';
    const related=selectedProof?renderLinkCollection({label:'Continue the analysis',items:chapterRelatedLinks,variant:'related'}):'';
    return `<section class="cheat-section site-chapter-section${selectedProof?' site-design-proof-section':''}" id="section-${esc(section.id)}" data-section="${esc(section.id)}">${header}${hardware}<div class="concept-grid site-concept-grid">${section.concepts.map(concept=>renderConceptCard(concept,selectedProof)).join('')}</div>${related}</section>`;
  }).join('');
  const selectedSection=sections.find(section=>section.id===route.params.get('section'));
...
function renderTool(route){
  ...
  const context=proof?`<section class="site-context-grid" aria-label="Calculator context">${renderLinkCollection({label:'Related concepts',items:calculatorRelatedLinks,variant:'related'})}${renderLinkCollection({label:'Applied hardware',items:chapterHardwareLinks.slice(0,3),variant:'hardware'})}</section>`:'';
...
function renderReferences(){const page=`${renderBreadcrumbs([{label:'Home',href:'#/'},{label:'Assumptions and references'}])}${intro({eyebrow:'Methodology and source trail',title:'Know what the model<br>assumes before trusting it.',lede:'References, nomenclature, unit conventions, verification principles, and calculator limitations are part of the product—not footnotes hidden after the answer.',aside:'<p>Representative material properties and compact formulas are for screening. Production work must use controlled methods and traceable inputs.</p>',metrics:[{value:referenceGroups.length,label:'reference groups'},{value:glossary.length,label:'defined terms'},{value:'Explicit',label:'validity labels'}],buttons:[{label:'Open tool library',href:'#/tools'},{label:'Print this page',action:'print',secondary:true}]})}<div class="reference-layout site-reference-layout"><nav class="reference-nav" aria-label="Reference sections"><a href="#reference-method">Methodology</a>${referenceGroups.map(g=>`<a href="#reference-${slug(g.group)}">${esc(g.group)}</a>`).join('')}<a href="#reference-glossary">Glossary</a></nav><div><section class="reference-section" id="reference-method"><h2>Methodology and verification</h2><p>Every calculator separates a governing model from its interface. Inputs are normalized to coherent units, invalid domains are rejected, limiting cases are checked, and outputs identify whether they are exact within the model, numerical, empirical, or screening-level.</p><ul><li>Match response quantity, units, frequency range, duration, and statistical basis before combining environments.</li><li>Use complex cross terms when sources are correlated.</li><li>Run convergence checks for FFT, SRS, VRS, FDS, modal truncation, and spatial-pattern truncation.</li><li>Check thin-structure, plane-wave, diffuse-field, weak-coupling, and high-modal-overlap assumptions rather than applying them by habit.</li><li>Verify material data at the applicable temperature, orientation, fabrication state, and uncertainty basis.</li></ul>${renderCallout({tone:'warning',label:'Scope',body:'this site is an analysis companion and calculation record—not an authority for qualification limits, certification, or program-controlled design allowables.'})}</section>${referenceGroups.map(g=>`<section class="reference-section" id="reference-${slug(g.group)}"><h2>${esc(g.group)}</h2><ul class="reference-list">${g.items.map(r=>`<li><strong>${esc(r.title)}</strong><span>${esc(r.note)}</span></li>`).join('')}</ul></section>`).join('')}<section class="reference-section" id="reference-glossary"><h2>Glossary</h2><dl class="glossary-grid">${glossary.map(([term,def])=>`<div class="glossary-item"><dt>${esc(term)}</dt><dd>${esc(def)}</dd></div>`).join('')}</dl></section></div></div>`;return renderPageShell(page,{variant:'references'});}
...
function render(){
  ...
  document.body.classList.toggle('site-system-chapter',first==='cheat-sheet');
