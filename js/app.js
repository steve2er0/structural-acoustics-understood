...
function renderConceptCard(concept, proof = false) {
  const interpretation = concept.interpretation
    ? (proof
      ? renderCallout({ tone: 'note', label: 'Engineering note', body: concept.interpretation })
      : `<p><strong>Physical interpretation.</strong> ${esc(concept.interpretation)}</p>`)
    : '';
  const mistake = concept.mistake
    ? (proof
      ? renderCallout({ tone: 'warning', label: 'Common mistake', body: concept.mistake })
      : `<div class="mini-callout"><strong>Common mistake:</strong> ${esc(concept.mistake)}</div>`)
    : '';
...
function renderCheat(route){
  ...
  const breadcrumbs=proof?renderBreadcrumbs([{label:'Home',href:'#/'},{label:'Hardware atlas',href:'#/cheat-sheet?section=shell-acoustics-deep-dive'},{label:'Cylindrical-shell structural acoustics'}]):'';
...
function renderTool(route){
  ...
  const assumptionRule=proof
    ? renderCallout({tone:'assumption',label:'Use rule',body:'A polished numerical result does not expand the validity of its governing model.'})
    : '<div class="mini-callout"><strong>Use rule:</strong> a polished numerical result does not expand the validity of its governing model.</div>';
...
function render(){
  ...
  const designProof=isChapterDesignProof(route)||isCalculatorDesignProof(route);
  document.body.classList.toggle('home-route',!first);
  document.body.classList.toggle('site-system-route',designProof);
  document.body.classList.toggle('site-system-chapter',isChapterDesignProof(route));
  document.body.classList.toggle('site-system-calculator',isCalculatorDesignProof(route));
