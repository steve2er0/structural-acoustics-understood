...
const definitions = {
  ...
  'acoustic-treatment': {
    ...
    compute(v) {
      const s = acousticTreatmentState({ frequency: v.frequency, flowResistivity: v.flow_resistivity, thicknessMm: v.thickness, airGapMm: v.air_gap, coverage: v.coverage, blanketMass: v.blanket_mass, baselineAbsorption: v.baseline_absorption });
      return { summary: [stat('Material absorption', s.normalAbsorption), stat('Installed-area absorption', s.installedAbsorption), stat('Decay reduction', s.decayReductionDb, 'dB'), stat('Limp-mass TL screen', s.massLawTl, 'dB'), stat('Combined insertion-loss screen', s.insertionLoss, 'dB'), stat('Depth quarter-wave frequency', s.quarterWaveFrequency, 'Hz'), stat('Regime', s.regime)], interpretation: `At ${s.frequency.toFixed(0)} Hz, the material absorbs ${(100 * s.normalAbsorption).toFixed(0)}% at normal incidence, but coverage reduces the installed average to ${(100 * s.installedAbsorption).toFixed(0)}%. The treatment is in the ${s.regime}.`, engineeringConsiderations: programChecks('Fairing blankets and liners must retain acoustic impedance after compression, seams, purge flow, temperature, contamination, and flight installation.'), warnings: [s.frequency < 0.45 * s.quarterWaveFrequency ? 'The treatment is shallow relative to wavelength; low-frequency benefit is thickness limited.' : 'Validate diffuse-incidence absorption and installed insertion loss in a representative cavity.'], plots: [{ title: 'Porous absorption versus frequency', xLabel: 'Frequency (Hz)', yLabel: 'Absorption coefficient', xScale: 'log', traces: [trace('Material', s.frequencies, s.absorptionCurve, { emphasis: true }), trace('Installed-area average', s.frequencies, s.installedCurve)] }] };
    }
  },
