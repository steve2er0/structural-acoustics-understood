...
export function displayEngineeringResult(result,system){
  ...
  const schematics=(result.schematics||[]).map(schematic=>({...schematic,svg:schematic.svgByUnit?.[system]||schematic.svg}));
  return {...result,values:(result.values||[]).map(convertValue),plots,rangeCharts,tables,schematics,interpretation:result.interpretationByUnit?.[system]||result.interpretation};
}
