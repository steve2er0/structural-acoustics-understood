...
export function displayEngineeringResult(result,system){
  ...
  const visuals=(result.visuals||[]).map(visual=>({...visual,displaySystem:system}));
  return {...result,values:(result.values||[]).map(convertValue),plots,rangeCharts,tables,visuals,interpretation:result.interpretationByUnit?.[system]||result.interpretation};
}
